...
parser.add_argument('-f', #fragment info
                    '--fragment',
                    default=False, # Default is do not require fragment info
                    dest='frag',
                    help='specify if require coordinates of fragment',
                    action='store_true', 
                    )
...
with open(args.samfile, mode="r") as sam_file:
    for i, line in enumerate(sam_file):
        if not line.startswith('@'):
            ...
            chr = column[2]
            if chr != '*':
                start = int(column[3]) -1 -int(args.pad)
                end = int(column[3]) + len(column[9]) + int(args.pad)
                name = column[0]
                score = column[4]
                if args.frag == True: # for fragments
                    length = int(column[8])
                    if length > 0:
                        end = int(column[3]) + length + int(args.pad)
                        bed_file.write(f'{chr}\t{start}\t{end}\t{name}\t{score}\t{length}\t.\n')
                else: # for reads
