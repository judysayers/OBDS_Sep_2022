...
> Answer: Data dimenions 150,5; comma separated; 1 character column called species, 4 floating point columns 
...
> Answer: Tibble, 150 x 5, 4x numeric, 1x character
...
> Answer: does ther job but not the prettiest as tab separated so alignment is off
...
```{r}
iris_raw %>% group_by(species) %>%
  summarise(across(tidyselect::where(is.numeric), mean))
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_for_ggplot <- ggplot(iris_raw %>% mutate(sepal_area = sepal_length * sepal_width) %>%
  select(species, sepal_area), aes(x = sepal_area, fill = species)) +
  geom_histogram() +
  labs(title = "Histogram of sepal area", y = "Frequency", x = "Sepal area") +
  theme_classic() +
...
```{r}
iris_long <- iris_raw %>% pivot_longer(cols = !species, names_to = "variable", values_to = "value")
    
iris_long # or cols = select(starts_with("petal" | "sepal") ? or cols = c("","")
```

**What information have we lost in the process? What could we do to remedy the issue?**

> Answer: info as to which flower sepal_length goes with petal_width etc.

```{r}
# Copy the code chunk above and refine to address the issue
iris_long2 <- iris_raw %>% 
  mutate(ID = seq(1, nrow(iris_raw))) %>%
  pivot_longer(cols = !species & !ID, names_to = "variable")
...
iris %>% 
    ...
    as_tibble() #NB must use [ ] around . as the dot has a meaning on its own
```
...
```{r}
library(readxl)
excel_path <- "/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx"
 # lapply(excel_sheets(excel_path), read_excel, path = excel_path) #to read in all sheets in one command
exprs_assay <- read_xlsx(excel_path, sheet = 1)
...
```{r}
exprs_assay_long <- exprs_assay %>% pivot_longer(
  cols = !gene, names_to = "sample", values_to = "count"
)
...
```{r}
ggplot(exprs_assay_long, aes(x = count)) +
  geom_histogram() +
  facet_wrap(~gene) +
  labs(title = "Histogram of gene expression levels", y = "Number of samples", x = "Gene expression") +
  theme_classic() +
  theme(text = element_text(size = 16)) +
  theme(plot.title = element_text(hjust = 0.5)) 
...
```{r}
sample_info <- read_xlsx(excel_path, sheet = "sample_info")
sample_info
```
...
```{r}
sample_info <- sample_info %>% mutate(across(c(grade, T, N, M),
                                      as.factor))
...
```{r}
exprs_sampleinfo <- sample_info %>% inner_join(exprs_assay_long, by = c("unique_patient_ID" = "sample"))
...
```{r}
exprs_sampleinfo %>% group_by(sex, gene) %>%
